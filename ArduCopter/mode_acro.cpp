...
void ModeAcro::run()
{
    ...
    float target_roll_cd, target_pitch_cd, target_yaw_cd;
    get_pilot_desired_rates_cds(channel_roll->norm_input_dz(), channel_pitch->norm_input_dz(), channel_yaw->norm_input_dz(), target_roll_cd, target_pitch_cd, target_yaw_cd);
    ...
    if (g2.acro_options.get() & uint8_t(AcroOptions::RATE_LOOP_ONLY)) {
        ...
        attitude_control->input_rate_bf_roll_pitch_yaw_2_cds(target_roll_cd, target_pitch_cd, target_yaw_cd);
    } else {
        // send rate commands to attitude controller with attitude stabilization
        attitude_control->input_rate_bf_roll_pitch_yaw_cds(target_roll_cd, target_pitch_cd, target_yaw_cd);
    }
...
void ModeAcro::get_pilot_desired_rates_cds(float roll_norm_in, float pitch_norm_in, float yaw_norm_in, float &roll_out_cds, float &pitch_out_cds, float &yaw_out_cds)
{
    float rate_delta_max_cds;
    Vector3f rate_ef_level_cds, rate_bf_level_cds, rate_bf_request_cds;

    // apply circular limit to pitch and roll inputs
    float norm_in_length = norm(pitch_norm_in, roll_norm_in);
    ...
    rate_bf_request_cds.x = g2.command_model_acro_rp.get_rate() * 100.0 * input_expo(roll_norm_in, g2.command_model_acro_rp.get_expo());

    // pitch rate request with input expo applied
    rate_bf_request_cds.y = g2.command_model_acro_rp.get_rate() * 100.0 * input_expo(pitch_norm_in, g2.command_model_acro_rp.get_expo());

    // yaw rate request with input expo applied
    rate_bf_request_cds.z = g2.command_model_acro_y.get_rate() * 100.0 * input_expo(yaw_norm_in, g2.command_model_acro_y.get_expo());
    ...
    if (g.acro_trainer != (uint8_t)Trainer::OFF) {
        ...
        if (g.acro_trainer == (uint8_t)Trainer::LIMITED) {
            ...
            float acro_level_mix = constrain_float(1-float(MAX(MAX(abs(roll_norm_in), abs(pitch_norm_in)), abs(yaw_norm_in))/4500.0), 0, 1) * ahrs.cos_pitch();
